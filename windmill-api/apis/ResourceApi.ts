...
export class ResourceApiRequestFactory extends BaseAPIRequestFactory {
     ...
     * @param path 
     */
    public async existsResource(workspace: string, path: string, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // verify required parameter 'workspace' is not null or undefined
        if (workspace === null || workspace === undefined) {
            throw new RequiredError("ResourceApi", "existsResource", "workspace");
        }


        // verify required parameter 'path' is not null or undefined
        if (path === null || path === undefined) {
            throw new RequiredError("ResourceApi", "existsResource", "path");
        }


        // Path Params
        const localVarPath = '/w/{workspace}/resources/exists/{path}'
            .replace('{' + 'workspace' + '}', encodeURIComponent(String(workspace)))
            .replace('{' + 'path' + '}', encodeURIComponent(String(path)));
     ...
     * @param path 
     */
    public async existsResourceType(workspace: string, path: string, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // verify required parameter 'workspace' is not null or undefined
        if (workspace === null || workspace === undefined) {
            throw new RequiredError("ResourceApi", "existsResourceType", "workspace");
        }


        // verify required parameter 'path' is not null or undefined
        if (path === null || path === undefined) {
            throw new RequiredError("ResourceApi", "existsResourceType", "path");
        }


        // Path Params
        const localVarPath = '/w/{workspace}/resources/type/exists/{path}'
            .replace('{' + 'workspace' + '}', encodeURIComponent(String(workspace)))
            .replace('{' + 'path' + '}', encodeURIComponent(String(path)));
...
export class ResourceApiResponseProcessor {
     ...
     * to the expected objects
     *
     * @params response Response returned by the server for a request to existsResource
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async existsResource(response: ResponseContext): Promise<boolean > {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
     ...
     * to the expected objects
     *
     * @params response Response returned by the server for a request to existsResourceType
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async existsResourceType(response: ResponseContext): Promise<boolean > {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: boolean = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
